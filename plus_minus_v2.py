...
class PlusMinusPredictorV2(nn.Module):
    """Version 2.0: Improved model with team context and batch normalization."""
...
class ConfigV2:
    ...
    HIDDEN_LAYERS = [256, 128, 64]  # Larger hidden layers
...
def train_model_v2():
    """Train Version 2.0 model using pre-existing CSV files."""
    ...
    # Create dataloaders
    train_loader = DataLoader(train_dataset, batch_size=ConfigV2.BATCH_SIZE, shuffle=True)
    test_loader = DataLoader(test_dataset, batch_size=ConfigV2.BATCH_SIZE)
    
    # Initialize model
    model = PlusMinusPredictorV2(
        input_size=X_train.shape[1],
        hidden_layers=ConfigV2.HIDDEN_LAYERS,
        dropout_rate=ConfigV2.DROPOUT_RATE
    ).to(ConfigV2.DEVICE)
    
    # Loss function and optimizer
    criterion = nn.MSELoss()
    optimizer = optim.Adam(model.parameters(), lr=ConfigV2.LEARNING_RATE)
    ...
    for epoch in range(ConfigV2.NUM_EPOCHS):
        ...
        for X_batch, y_batch in train_loader:
            X_batch, y_batch = X_batch.to(ConfigV2.DEVICE), y_batch.to(ConfigV2.DEVICE)
        ...
        print(f'Epoch {epoch+1}/{ConfigV2.NUM_EPOCHS}: Train Loss: {train_loss:.4f}, Val Loss: {val_loss:.4f}')
        ...
        if val_loss < best_val_loss:
            ...
            patience_counter += 1
            if patience_counter >= ConfigV2.EARLY_STOPPING_PATIENCE:
    ...
    with torch.no_grad():
        for X_batch, y_batch in test_loader:
            X_batch = X_batch.to(ConfigV2.DEVICE)
            outputs = model(X_batch)
    ...
    print(f'\nFinal Test Results for Version 2.0:')
    print(f'MSE: {mse:.4f}')
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('training_validation_loss_v2.png')  # Save the plot as an image
    ...
    plt.grid(True)
    plt.savefig('predicted_vs_actual_v2.png')  # Save the plot as an image
    plt.show()  # Display the plot
